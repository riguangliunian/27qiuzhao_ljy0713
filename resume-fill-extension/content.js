...
async function fillResumeForm() {
  ...
  let filled = await fillVisibleControls(document, rules)
...
async function fillVisibleControls(root, rules) {
  let filled = 0
  ...
  // Second pass: try custom dropdowns (Arco / Ant Design / Element UI)
  filled += await fillCustomSelects(root, rules)
  return filled
}
...
async function fillExperienceSections(templates, baseRules) {
  ...
  for (const template of templates) {
    ...
    button.click()
    sections += 1
    await sleep(Number(template.waitMs || 800))

    // Retry scope detection if the modal/panel hasn't appeared yet
    let scope = findLikelyActiveScope()
    if (scope === document) {
      await sleep(600)
      scope = findLikelyActiveScope()
    }

    const scopedRules = normalizeRules(template.rules || []).filter((rule) => rule.value && Array.isArray(rule.keywords))
    filled += await fillVisibleControls(scope || document, [...scopedRules, ...baseRules])

    // Click confirm/save button if configured, then wait for modal to close
    if (template.confirmKeywords?.length) {
      const confirmBtn = findConfirmButton(scope || document, template.confirmKeywords)
      if (confirmBtn) {
        confirmBtn.click()
        await sleep(600)
      }
...
function findConfirmButton(scope, confirmKeywords) {
...
// ── Custom dropdown support (Arco Design / Ant Design / Element UI) ──────────

async function fillCustomSelects(root, rules) {
  let filled = 0
  // Find inputs that are combobox triggers inside custom select containers
  const seen = new Set()
  const candidates = [...root.querySelectorAll(
    '[role=combobox], [class*=select] > input, [class*=Select] > input, ' +
    '[class*=select-selector] input, [class*=SelectSelector] input'
  )].filter((el) => {
    if (!isVisible(el) || el.disabled || hasValue(el) || seen.has(el)) return false
  ...
  for (const element of candidates) {
    ...
    const rule = matchRule(text, rules)
    if (!rule) continue
    const ok = await tryFillCustomDropdown(element, rule.value)
    if (ok) filled += 1
  }
  return filled
}

async function tryFillCustomDropdown(trigger, value) {
  const container = trigger.closest(
    '[class*=select], [class*=Select], [class*=picker], [class*=Picker], [class*=cascader], [class*=Cascader]'
  ) || trigger
  ...
  const popup = findVisibleDropdownPopup()
  if (!popup) {
    document.dispatchEvent(new KeyboardEvent("keydown", { key: "Escape", bubbles: true, cancelable: true }))
    return false
  }

  const wanted = cleanText(String(value))
  const options = [...popup.querySelectorAll(
    '[role=option], [class*=option-item], [class*=OptionItem], [class*=select-item], li[class*=option], li[class*=Option]'
  )]
...
function findVisibleDropdownPopup() {
  const selectors = [
    "[role=listbox]",
    ".ant-select-dropdown",
    ".el-select-dropdown",
...
function findLikelyActiveScope() {
  const modal = [...document.querySelectorAll(
    "[role=dialog], .modal, .ant-modal, .el-dialog, .arco-modal, .semi-modal, " +
    "[class*=modal-content], [class*=ModalContent], [class*=drawer], [class*=Drawer]"
...
function findFillableControls(root) {
  return [...root.querySelectorAll(
    "input:not([type=file]):not([type=hidden]):not([type=submit]):not([type=button]):not([type=reset]), " +
...
function fillControl(element, value) {
  if (element.tagName === "SELECT") {
    ...
    element.focus()
    // Use native setter so React/Vue state updates correctly
